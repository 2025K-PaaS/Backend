...
def my_balance(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return PointBalanceOut(balance=point_service.get_balance(db, current_user.id))
...
def my_history(
    ...
    items, next_before_id = point_service.get_history(db, current_user.id, limit, before_id)
    return PointHistoryOut(items=items, next_before_id=next_before_id)

@router.post("/grant", response_model=PointBalanceOut, dependencies=[Depends(require_admin_key)])
def grant_points(payload: GrantPointsIn, db: Session = Depends(get_db)):
    new_balance = point_service.award(
        db,
    ...
    db.commit()
    return PointBalanceOut(balance=new_balance)
